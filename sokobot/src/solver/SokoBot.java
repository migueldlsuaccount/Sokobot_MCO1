...
// Add these penalty calculation methods to your SokoBot class

    // 1. Clustered Boxes Penalty - penalizes boxes that are too close together
    private int calculateClusteredBoxesPenalty() {
        int penalty = 0;
        for (int i = 0; i < this.box_count; i++) {
            for (int j = i + 1; j < this.box_count; j++) {
                int distance = computeManhattanDistance(
                        this.Position_boxes.get(i),
                        this.Position_boxes.get(j)
                );
  ...
  */
    ...
    private void initializeMapData(int width, int height, char[][] mapData, char[][] itemsData)
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                if (mapData[i][j] == '.')
                {
                    Position_goals.add(new int[]{i, j});
                }
                if (itemsData[i][j] == '$')
                {
        ...
        for (int i = 1; i < height - 1; i++)
        {
            for (int j = 1; j < width - 1; j++)
            {
                ...
                if (isBasicDeadlock(i, j, mapData))
                {
        ...
        this.boxMap.clear();
        for (int[] box : Position_boxes)
        {
            boxMap.add(box[0] + "," + box[1]);
        }

    }

    private boolean isWall(int i, int j, char[][] mapData)
    {
        return i < 0 || i >= height || j < 0 || j >= width || mapData[i][j] == '#';
    }

    private boolean isBasicDeadlock(int i, int j, char[][] mapData)
    {
        boolean wallUp = isWall(i-1, j, mapData);
        boolean wallDown = isWall(i+1, j, mapData);
        boolean wallLeft = isWall(i, j-1, mapData);
        boolean wallRight = isWall(i, j+1, mapData);
        ...
        if (isTunnelDeadlock(i, j, mapData))
        {
            return true;
        }
    ...
    private boolean isTunnelDeadlock(int i, int j, char[][] mapData)
    {
        if (isWall(i-1, j, mapData) && isWall(i+1, j, mapData))
        {
            if (isTunnelDeadEnd(i, j, 0, -1, mapData) && isTunnelDeadEnd(i, j, 0, 1, mapData))
            {
                return true;
            }
        }

        if (isWall(i, j-1, mapData) && isWall(i, j+1, mapData))
        {
            if (isTunnelDeadEnd(i, j, -1, 0, mapData) && isTunnelDeadEnd(i, j, 1, 0, mapData))
            {
                return true;
            }
        }
    ...
    private boolean isTunnelDeadEnd(int startI, int startJ, int di, int dj, char[][] mapData)
    {
        int i = startI + di;
        int j = startJ + dj;

        while (!isWall(i, j, mapData))
        {
            if (mapData[i][j] == '.')
            {
                return false;
    ...
    private boolean hasExit(int i, int j, int moveDi, int moveDj, char[][] mapData)
    {
        if (moveDi != 0)
        {
            return !isWall(i, j-1, mapData) || !isWall(i, j+1, mapData);
        }
        else
        {
            return !isWall(i-1, j, mapData) || !isWall(i+1, j, mapData);
        }
